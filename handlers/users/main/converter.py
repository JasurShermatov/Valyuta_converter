...
async def safe_api_call(func, *args, **kwargs) -> Tuple[Optional[float], Optional[datetime]]:
...
async def process_conversion(amount: float, from_currency: str, to_currency: str) -> tuple[str, str, datetime]:
    """Bir valyutani konvertatsiya qilish"""
    try:
        if not validate_currency(from_currency) or not validate_currency(to_currency):
            raise ValueError(f"Noto'g'ri valyuta kodi: {from_currency} yoki {to_currency}")

        rate, update_time = await safe_api_call(currency_api.get_rate, from_currency, to_currency)

        if rate is None:
            raise ValueError(f"Valyuta kursi topilmadi: {from_currency} -> {to_currency}")
    ...
    try:
        ...
        if currency in selected:
            ...
            if len(selected) >= 10:  # Limit the number of selections
                await callback.answer("❌ Ko'pi bilan 10 ta valyuta tanlash mumkin", show_alert=True)
                return
        ...
        await callback.message.edit_text(text, reply_markup=create_convert_keyboard(from_currency, selected))
    except Exception as e:
...
async def process_amount(message: Message, state: FSMContext):
    try:
        ...
        for to_currency in selected_currencies:
            result, status, update_time = await process_conversion(amount, from_currency, to_currency)
            results.append(result)
            if status == "success":
                latest_update = update_time

        time_info = f"\n\n🕐 Yangilangan vaqt: {latest_update.strftime('%H:%M:%S')}" if latest_update else ""
        message_text = f"💱 Konvertatsiya natijasi:\n\n" + "\n\n".join(results) + time_info
